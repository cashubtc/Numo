    ...
    fun draw(
        ...
        oldGlyphs.forEachIndexed { i, glyph ->
            ...
            if (unchanged) {
                ...
                paint.alpha = (originalAlpha * (1f - progress)).toInt()
                canvas.drawText(from, i, i + 1, drawX, baseline - travel * progress, paint)
            }
        }
        newGlyphs.forEachIndexed { i, glyph ->
            ...
            if (origin == null || oldGlyphs[origin].character != glyph.character) {
                ...
                paint.alpha = (originalAlpha * progress).toInt()
                canvas.drawText(to, i, i + 1, drawX, baseline + travel * (1f - progress), paint)
            }
        }
        paint.alpha = originalAlpha
