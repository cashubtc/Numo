...
| A tap. A payment. | Let customers pay with a tap from a compatible bitcoin wallet. | A stationary photographic handset shows the shipping white-theme POS entering $3.50, the payment request, and payment received. An iPhone showing the user-supplied wallet scan sheet approaches diagonally at a fixed shallow angle, pauses, and withdraws as confirmation appears. | 8.8 s |
...
The tap sequence keeps the merchant phone fixed and uses 220 ms screen crossfades. The customer phone holds a constant -5° angle, approaches over 640 ms, rests for 1.12 seconds, and withdraws over 520 ms. Confirmation appears during withdrawal so it becomes visible as the foreground phone clears the terminal. The supplied wallet screenshot remains intact. The phones are still photographic 2D composites; this pass refines their choreography without introducing simulated 3D rotation.
