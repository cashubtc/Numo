...
The Impeccable typeset passes preserve Android sans-serif and the existing condensed wordmark. Scoped text roles use 32/38sp medium display headlines at -0.02em tracking (four short marketing lines earn display scale; tracking tightens with size), 16/24sp regular descriptions at 0.01em to compensate light-on-dark reading, 17/22sp medium actions, and 12/17sp regular legal text whose links use real sans-serif-medium rather than synthetic bold. MaterialTextView supplies compatible line height on older Android versions. Headings and descriptions balance wrapping without hyphenation, descriptions are bounded to 360dp, and the 120×54dp wordmark stays centered in its header with an additional 24dp of top breathing room below the safe area. The small green panel captions use #008935 on white, above 4.5:1 contrast. The type detector returns no findings for these targets; it is a web detector, so native source and physical render review provide the actual assessment.
